...
public class Surface {
     ...
     *            inserted
     */
    public void insertEntry(Tile tileSort, Position pos) {
        Tile newTile = new Tile(tileSort.getId(), tileSort.getRows(), tileSort.getCols());
        for (int i = pos.getRow(); i <= pos.getRow() + tileSort.getRows() - 1; i++) {
            for (int j = pos.getCol(); j <= pos.getCol() + tileSort.getCols() - 1; j++) {
                fields[i][j] = newTile;
            }
        }
    }

    public void removeEntry(Tile tileSort, Position pos) {
        for (int i = pos.getRow(); i <= pos.getRow() + tileSort.getRows() - 1; i++) {
            for (int j = pos.getCol(); j <= pos.getCol() + tileSort.getCols() - 1; j++) {
                fields[i][j] = null;
            }
